...
chi_sq_best = SelectKBest(score_func=chi2, k = 12000)
...
xgb = xgboost.XGBClassifier(max_depth = 14, n_estimators = 299,
...
submission.to_csv(submission_name + '_NN_15000Features.csv', index=False)

submission = pd.DataFrame(np.round(y_xgb_test_predictions, 4), index=y_df[pd.isnull(y_df.TripType)].index, columns = col_names)
submission.reset_index(inplace = True)
submission.to_csv(submission_name + '_XGB_15000Features.csv', index=False)
...
submission.reset_index(inplace = True)
submission.to_csv(submission_name + '_log_ensembl_15000Features-Notebook.csv', index=False)
...
submission.reset_index(inplace = True)
submission.to_csv(submission_name + '_avg_ensembl_15000Features-Notebook.csv', index=False)
